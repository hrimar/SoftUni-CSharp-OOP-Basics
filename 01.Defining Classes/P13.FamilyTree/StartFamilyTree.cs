...
class StartFamilyTree
{
    static void Main()       // баааси задачата... Виж и новото реш-е от тема 02.- за 07!
    {
        ...
        {
            string[] tokens = input.Split(" - "); // new in .NET Core
            if (tokens.Length > 1)
            {
    ...
    private static void SetChild(List<Person> familyTree, Person perentPerson, string child)
    {
        ...
        perentPerson.Children.Add(childPerson);
        childPerson.Parents.Add(perentPerson);
        familyTree.Add(childPerson);
    }

    static bool IsBirthday(string personInput)
    {
